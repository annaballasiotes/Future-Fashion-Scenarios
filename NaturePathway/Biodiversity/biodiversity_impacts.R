...
#contraction_summary <- read.csv("Future-Fashion-Scenarios/NaturePathway/Biodiversity/biodiversity_summary_reduction.csv")

commodity.labs <- c("cotton", "wool", "cashmere")
names(commodity.labs) <- c("Cotton", "Wool", "Cashmere")

contraction_summary
labels = c("Cotton", "Wool", "Cashmere")

contraction_summary$commodity <- factor(contraction_summary$commodity, levels = c("cotton", "wool", "cashmere"), 
                                             labels = c("Cotton", "Wool", "Cashmere"))

ggplot(contraction_summary, aes(x = pct, y = total_species, color = scenario)) +
  #geom_point() +
  ...
  theme(text = element_text(family = "Proxima Nova", size = 14)) +
  labs(x = "% Land Footprint Removed", title = "IUCN Red List Species in Reduced Footprint", y = "Species Count", ) +
  geom_line(size = 1)
